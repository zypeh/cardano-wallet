...
conf = Config
    ...
    , genSlotAfter = \t -> toEnum <$> choose (fromEnum t + 1, fromEnum t + 10)
...
units = withInitializedWalletProp $ \wid runQ -> do
    [p0, p1, _p2] <- liftIO $ generate $
    ...
    runQ $ runStoreLaw (mkStoreDelegations wid) $ do
        unit "reg-dereg" $ do
            applyS $ Register 0
            applyS $ Deregister 0
