...
spec = describe "Cardano.Wallet.Primitive.Types" $ do
  ...
  parallel $ describe "Slot arithmetic" $ do
    ...
    it "t < slotStartTime slotMinBound => slotFloor t == Nothing" $
      withMaxSuccess 1000 $
        property $ \sps t ->
          StartTime (getUniformTime t)
            < getGenesisBlockDate sps
            ==> slotFloor sps (getUniformTime t)
            === Nothing
    ...
    it "t < slotStartTime slotMinBound => slotCeiling t == slotMinBound" $
      withMaxSuccess 1000 $
        property $ \sps t ->
          StartTime (getUniformTime t)
            < getGenesisBlockDate sps
            ==> slotCeiling sps (getUniformTime t)
            === slotMinBound
    ...
    it
      ...
      $ \sps r -> do
        ...
        slotStartTime sps slotMinBound
          `isAfterRange` r'
    ...
    it
      ...
      $ property
      $ \(sps, slot) (NonNegative (n :: Int)) ->
        flatSlot (getEpochLength sps) slot
          >= fromIntegral n
  ...
  where
    ...
    prop =
      balance (u `excluding` ins)
        === balance u
        `TokenBundle.unsafeSubtract` balance (u `restrictedBy` ins)
