...
import Cardano.Wallet.Primitive.Types.UTxOIndex.Internal qualified as UTxOIndex
...
prop_filter_disjoint f i =
    checkCoverage_filter_partition f i $
        UTxOIndex.filter f i
            `UTxOIndex.disjoint` UTxOIndex.filter (not . f) i
            === True
  ...
  where
    i1 `isNonEmptyProperSubsetOf` i2 =
        not (UTxOIndex.null i1)
            && UTxOIndex.toMap i1
            `Map.isSubmapOf` UTxOIndex.toMap i2
            && i1
            /= i2
