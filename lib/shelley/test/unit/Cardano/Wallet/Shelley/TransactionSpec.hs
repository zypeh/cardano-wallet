...
import Cardano.Wallet.Primitive.Slotting
    ( dummyEraHistory, dummyForkEraHistory )
    ...
    describe "when passed unresolved inputs" $ do
        ...
        describe "with redeemers" $
            it "fails with RedeemerPointsToUnknownScriptHash" $ do
                ...
                let RedeemerSpending _ inp = head $ redeemers pingPong_2

                shouldBe (balanceTx (withNoUTxO pingPong_2))
                    . Left
                    . ErrBalanceTxAssignRedeemers
                    . ErrAssignRedeemersScriptFailure
                    $ Cardano.ScriptErrorMissingTxIn (toCardanoTxIn inp)
     ...
     shrink (Cardano.TxOutValue Cardano.MultiAssetInBabbageEra val) =
         map
             (Cardano.TxOutValue Cardano.MultiAssetInBabbageEra
                 . toCardanoValue)
             (shrink $ fromCardanoValue val)
     shrink (Cardano.TxOutValue Cardano.MultiAssetInMaryEra val) =
         map
...
instance IsCardanoEra era => Arbitrary (PartialTx era) where
    arbitrary = do
        let era = Cardano.cardanoEra
...
shrinkTx :: IsCardanoEra era => Cardano.Tx era -> [Cardano.Tx era]
shrinkTx (Cardano.Tx bod wits) =
    [ Cardano.Tx bod' wits | bod' <- shrinkTxBody bod ]
...
shrinkTxBody
    ...
    => Cardano.TxBody era
    -> [Cardano.TxBody era]
shrinkTxBody = case cardanoEra @era of
    Cardano.AlonzoEra -> shrinkTxBodyAlonzo
    Cardano.BabbageEra -> shrinkTxBodyBabbage
    e -> error $ "shrinkTxBody not implemented for " <> show e
    ...
    = withMaxSuccess 1000 $ do
        ...
        case res of
            ...
            Left ErrBalanceTxConflictingInputResolution ->
                label "conflicting input resolution" $ property True
